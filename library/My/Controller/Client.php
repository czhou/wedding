<?php
class My_Controller_Client extends Zend_Controller_Action {
	...
	public function init() {
		...
		Zend_Layout::getMvcInstance ()->setLayout ( "client.layout" );
		$this->view->menuClass = array ();
		$this->view->logger = $this->logger;
		if (! $this->auth->getIdentity () && $this->_request->getActionName () != "login") {
			$this->_redirect ( "/client/index/login" );
		} else {
			...
			if ($role){
				$this->_permittions = $role->getFrontEndActions();
			}
	...
	public function hasAccessTo($controllerName, $actionName){
		...
		$actionCode = "f|".$controllerName."|".$actionName;
